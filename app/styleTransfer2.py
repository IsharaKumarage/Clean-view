...
def preprocess_image(image_path, target_size=(224, 224)):
    ...
    img = resnet50.preprocess_input(img)  # Preprocess using ResNet50 preprocessing
    return img
...
def style_transfer(content_path, style_path, iterations=100):
    ...
    resnet = resnet50.ResNet50(weights="imagenet", include_top=False)  # Load ResNet50 model
    content_layer = 'conv4_block6_out'  # A suitable content layer from ResNet-50
    style_layers = ['conv1_conv', 'conv2_block3_out', 'conv3_block4_out', 'conv4_block6_out', 'conv5_block3_out']

    # Extract content features
    content_model = tf.keras.Model([resnet.input], [resnet.get_layer(content_layer).output])
    style_model = tf.keras.Model([resnet.input], [resnet.get_layer(layer).output for layer in style_layers])
    ...
    # Run the style transfer iterations
